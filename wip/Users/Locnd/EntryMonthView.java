...
public class EntryMonthView extends LinearLayout {
	...
	public EntryMonthView(Context context, String keyMonth) {
		...
		if (entrySet != null) {
			...
			// Prepare for entries in week.
			LinkedHashMap<String, ArrayList<Entry>> weekEntries = new LinkedHashMap<String, ArrayList<Entry>>();
			String key = getResources().getString(
					R.string.entry_view_week_item_title);

			// Prepare data for drawing chart.
			for (Entry entry : entrySet) {
				total += entry.getTotal();				
				String keyWeek = new StringBuilder(key).append(" ").append(
						DateTimeHelper.getWeekInMonth(entry.getDate()))
						.toString();
				
				if (!weekEntries.containsKey(keyWeek))
				{
					weekEntries.put(keyWeek, new ArrayList<Entry>());
				}
				
				weekEntries.get(keyWeek).add(entry);
			...
			for (String keyValue : weekEntries.keySet()) {
				entryDayList.addView(new EntryWeekView(getContext(), keyValue, weekEntries)); 
			}
