...
public class SyncSettingActivity extends Activity {
	...
	private String[] mScheduleWarnArr, mScheduleRingArr, mScheduleRemainArr,
			mBorrowWarnArr, mBorrowRingArr, mBorrowRemainArr;
	Thread runBackground = new Thread(new Runnable() {
		public void run() {
			try {
				Thread.sleep(10 * 3600000);
				if (AccountProvider.getInstance().currentAccount == null){
				AccountProvider.getInstance().currentAccount = AccountProvider
						.getInstance().getAccounts().get(0);
				}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mScheduleWarnArr = getResources().getStringArray(
				R.array.warning_schedule_warn_array);
		mScheduleRingArr = getResources().getStringArray(
				R.array.warning_ring_array);
		mScheduleRemainArr = getResources().getStringArray(
				R.array.warning_remain_array);
		mBorrowWarnArr = getResources().getStringArray(
				R.array.warning_borrow_warn_array);
		mBorrowRingArr = getResources().getStringArray(
				R.array.warning_ring_array);
		mBorrowRemainArr = getResources().getStringArray(
				R.array.warning_remain_array);
	...
	private DialogInterface.OnClickListener itemClicked = new DialogInterface.OnClickListener() {
		public void onClick(DialogInterface dialog, int whichButton) {
			...
			// ArrayAdapter<String> scheduleWarnAdapter = new
			// ArrayAdapter<String>(
